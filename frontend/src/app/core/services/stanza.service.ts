...
export class StanzaService {
    ...
    createStanza(stanza: Partial<Stanza>): Observable<Stanza> {
        return from(this.axiosService.post<Stanza>(`${this.baseUrl}/creaStanza`, stanza));
    }

    updateStanza(id: number, updates: Partial<Stanza>): Observable<Stanza> {
        return from(this.axiosService.put<Stanza>(`${this.baseUrl}/aggiornastanza/${id}`, updates));
    }

    deleteStanza(id: number): Observable<void> {
        return from(this.axiosService.delete(`${this.baseUrl}/eliminastanza/${id}`)).pipe(
            map(() => void 0)
        );
    }
}
